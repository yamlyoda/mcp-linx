...
class PostgresPlugin(DiagnosticPlugin):
    ...
    def get_tools(self) -> list[PluginTool]:
        ...
        return [
            ...
            PluginTool("pg_slow_queries", "Медленные запросы (с_QUERY_STATS)", pg_slow_queries),
    ...
    async def initialize(self, config: PluginConfig) -> None:
        self._config = config
        # Подключение в синхронном режиме (psycopg2 не async)
    ...
    async def _connect_sync(self) -> None:
        ...
        self._conn.autocommit = True  # Для read-only запросов
