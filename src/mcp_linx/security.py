...
class SecurityGuard:
    ...
    def _is_readonly_command(self, command: str) -> bool:
        ...
        # Проверка на docker exec, kubectl и т.д.
        if base_command in ["docker", "kubectl"]:
            # docker exec / docker logs — read-only
            if len(tokens) > 1 and tokens[1] in ["exec", "logs", "inspect", "stats", "top", "port"]:
                return True
    ...
    def limit_log_lines(self, logs: str) -> str:
        ...
        if len(lines) > self._max_log_lines:
            return "\n".join(lines[:self._max_log_lines]) + f"\n... [truncated, total lines: {len(lines)}]"
        return logs
