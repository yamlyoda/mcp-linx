...
`plugins.enabled` выбирает только инициализацию. Все загруженные плагины продолжают
регистрировать MCP-инструменты и участвовать в health-check; исключённые могут
возвращать ошибки инициализации. Это не способ скрыть инструменты и не граница доступа.
...
- **Read-only режим**: Валидация команд блокирует команды записи (rm, write, mkfs, dd
  и др.) для командных адаптеров и инструментов. Это не общий запрет любых
  изменяющих состояние операций: некомандные пути (например Docker prune) ключ
  `readonly` не проверяют и опираются на собственное подтверждение `confirm`.
