...
- [x] **E7 (FIXED 2026-09-25). Диагностика прав и атрибутов файла по инциденту service.log.** Добавлен read-only tool `linux_file_diagnostics`: `stat`, `namei`, `lsattr` (детект `immutable`), mount/space и свойства unit-а; опциональная проверка `test -w` от service-user через отдельный строгий runner `runuser -u USER -- test -w PATH`, включаемая `plugins.linux.privileged_tools=true`. `chattr` намеренно не добавлен. Тесты: `tests/unit/test_linux_file_diagnostics.py`; docs: README EN/RU, `docs/skills/linux.md`, `docs/skills/workflows.md`, `settings.yaml`.
